...
module.exports = {
  ...
  async execute(client, message, args, Embed){
    ...
    if (process.env.ENABLE_EVAL !== 'true') {
      return message.channel.error(message, `Eval komutu güvenlik nedeniyle devre dışı. Kullanmak için ENABLE_EVAL=true tanımlanmalıdır.`, { timeout: 8000 });
    }
    ...
    try { 
      ...
      const safeOutput = token ? evaled.replace(new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), "[Secret Information]") : evaled;
      message.channel.send(safeOutput, { code: 'js', split: true });
