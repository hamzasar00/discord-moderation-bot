...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        await message.guild.members.ban(user.id, { reason: `${!reason ? `${message.author.tag} tarafından bilinmeyen bir nedenle kalıcı olarak yasaklanması istendi!` : `${message.author.tag} tarafından ${reason} sebebiyle kalıcı olarak yasaklanması istendi!`}` });
        ...
        if(log && client.channels.cache.get(log)) client.channels.cache.get(log).send(Embed.setColor('#FF0000').setFooter('').setImage(banGifs.random()).setDescription(`
...
**Yasaklanma Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
        ...
        if(pointLog && client.channels.cache.get(pointLog)) client.channels.cache.get(pointLog).send(`${banned ? banned : ``} ${user.toString()}, aldığınız \`#${penal.id}\` ID'li **ForceBan** cezası ile toplam **${point.penalPoint}** ceza puanına ulaştınız!`);
