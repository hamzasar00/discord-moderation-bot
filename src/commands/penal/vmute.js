...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        await user.roles.add(vmuteRoles);
        if(user.voice.channel && !user.voice.serverMute) await user.voice.setMute(true).catch(() => {});
        ...
        if(log && client.channels.cache.get(log)) client.channels.cache.get(log).send(Embed.setColor('#FF0000').setDescription(`
...
**Susturulma Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
        ...
        if(logs.pointLog && client.channels.cache.get(logs.pointLog)) client.channels.cache.get(logs.pointLog).send(`${muted ? muted : ``} ${user.toString()}, aldığınız \`#${penal.id}\` ID'li **VoiceMute** cezası ile toplam **${point.penalPoint}** ceza puanına ulaştınız!`);
        ...
        setTimeout(async () => {
            ...
            if(log && client.channels.cache.get(log)) client.channels.cache.get(log).send(Embed.setColor('#00FF00').setDescription(`
