...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        await user.roles.set(jailRoles);
        if(user.voice.channel) await user.voice.kick().catch(() => {});
...
**Jaillenme Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
        ...
        if(logs.pointLog && client.channels.cache.get(logs.pointLog)) client.channels.cache.get(logs.pointLog).send(`${jailed ? jailed : ``} ${user.toString()}, aldığınız \`#${penal.id}\` ID'li **TempJail** cezası ile toplam **${point.penalPoint}** ceza puanına ulaştınız!`);
        ...
        setTimeout(async () => {
            ...
            if(log && message.guild.channels.cache.get(log)) message.guild.channels.cache.get(log).send(Embed.setColor('#00FF00').setDescription(`
