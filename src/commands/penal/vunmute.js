...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        if(!penal) return message.channel.error(message, `Bu üye için aktif bir ses susturma ceza kaydı bulunamadı.`, { timeout: 8000, react: true });
        await user.roles.remove(vmuteRoles);
        if(user.voice.channel && user.voice.serverMute) await user.voice.setMute(false);
