...
**Jaillenme Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
        ...
        if(logs.pointLog && client.channels.cache.get(logs.pointLog)) client.channels.cache.get(logs.pointLog).send(`${jailed ? jailed : ``} ${user.toString()}, aldığınız \`#${penal.id}\` ID'li **Jail** cezası ile toplam **${point.penalPoint}** ceza puanına ulaştınız!`);
