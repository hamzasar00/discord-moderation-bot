...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        if(log && client.channels.cache.get(log)) client.channels.cache.get(log).send(Embed.setDescription(`
...
**Uyarılma Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
        ...
        if(logs.pointLog && client.channels.cache.get(logs.pointLog)) client.channels.cache.get(logs.pointLog).send(`${warned} ${user.toString()}, aldığınız \`#${penal.id}\` ID'li **Warn** cezası ile toplam **${point.penalPoint}** ceza puanına ulaştınız!`);
