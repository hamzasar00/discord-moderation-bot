...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        await message.guild.members.ban(user.id, { reason: `${!reason ? `${message.author.tag} tarafından bilinmeyen bir nedenle yasaklanması istendi!` : `${message.author.tag} tarafından ${reason} sebebiyle yasaklanması istendi!`}` });
...
**Yasaklanma Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
        ...
        if(logs.pointLog && client.channels.cache.get(logs.pointLog)) client.channels.cache.get(logs.pointLog).send(`${banned ? banned : ``} ${user.toString()}, aldığınız \`#${penal.id}\` ID'li **Ban** cezası ile toplam **${point.penalPoint}** ceza puanına ulaştınız!`);
