...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        await user.roles.add(cmuteRoles);
...
**Susturulma Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
        ...
        if(logs.pointLog && client.channels.cache.get(logs.pointLog)) client.channels.cache.get(logs.pointLog).send(`${chatMuted ? chatMuted : ``} ${user.toString()}, aldığınız \`#${penal.id}\` ID'li **ChatMute** cezası ile toplam **${point.penalPoint}** ceza puanına ulaştınız!`);
