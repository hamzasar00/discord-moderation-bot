...
module.exports = {
    ...
    execute(client, message, args, Embed) {
        ...
        const applyMute = async (members, reason) => Promise.all(members.map(async (member, index) => {
            await client.wait((index + 1) * 300);
            await member.voice.setMute(true, { reason: !reason ? `${message.author.username} tarafından susturulması istendi` : reason });

            if(voiceLog) {
                const vLog = message.guild.channels.cache.get(voiceLog);
                if(vLog && vLog.type == 'text') await vLog.send(`${muted ? muted : ``} \`${member.displayName}\` üyesi \`${message.member.displayName}\` tarafından ${member.voice.channel.toString()} adlı ses kanalında **susturuldu!**`);
            }
        }));
        ...
        if(!message.member.voice.channel && args[0]) {
            ...
            message.channel.send(Embed.setDescription(`${channel.toString()} adlı ses kanalındaki üyeler ${!reason ? '' : `\`${reason}\` nedeniyle`} **susturulmaya** başlandı ${loading ? loading : ``} `)).then(async msg => {
                    await applyMute(channel.members.filter(member => !member.voice.serverMute), reason);
                    await msg.edit(Embed.setDescription(`${success ? success : ``} ${channel.toString()} adlı ses kanalındaki **${totalMembers}** üye ${!reason ? '' : `\`${reason}\` nedeniyle`} **susturuldu!**`));
