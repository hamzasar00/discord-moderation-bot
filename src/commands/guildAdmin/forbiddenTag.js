...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        if(['ekle', 'add'].some(arg => args[0].toLowerCase() == arg)) {
            ...
            await Promise.all(forbiddenMembers.map(async (member, index) => {
                await client.wait((index + 1) * 250);
                await member.roles.set(forbidRoles);
                ...
                const forbidChannelObject = message.guild.channels.cache.get(forbidChannel);
                const forbidLogObject = message.guild.channels.cache.get(forbidLog);
                if(forbidChannelObject) await forbidChannelObject.send(`${member.toString()}, kullanıcı adınızda sunucunun yasaklı tagları arasına eklenen \`${args[1]}\` tagını bulundurduğunuz için sunucuya erişiminiz kesildi`);
                if(forbidLogObject) await forbidLogObject.send(`\`${member.user.tag} (${member.user.id})\` kullanıcısının sunucuya erişimi kesildi! \`(Tag: ${args[1]})\``);
            }));
            ...
            if(!data || !data.forbiddenTags.includes(args[1])) return message.channel.error(message, Embed.setDescription(`Belirttiğin tag daha önce yasaklı taglar arasına eklenmemiş. Tagların listesini görmek için : \`${Prefix}yasaklıtag liste\``), { timeout: 8000, react: true });
            ...
            await Promise.all(forbiddenMembers.map(async (member, index) => {
                ...
                const forbidLogObject = message.guild.channels.cache.get(forbidLog);
                if(forbidLogObject) await forbidLogObject.send(`\`${member.user.tag} (${member.user.id})\` kullanıcısının sunucuya erişimi açıldı! \`(Tag: ${args[1]})\``);
                await member.roles.set(unregisterRoles);
            }));
...
${forbiddenMembers.size < 10 && forbiddenMembers.size ? `\`>\` **Taga sahip kişilerin listesi :**\n${forbiddenMembers.map(member => member.toString()).join(' , ')}` : !forbiddenMembers.size ? '' : `\`>\` **Taga sahip kişilerin listesini görmek için sayfayı çevirin**`}
            ...
            for (const [index, tag] of data.forbiddenTags.entries()) {
