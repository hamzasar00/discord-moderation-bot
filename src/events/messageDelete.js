...
module.exports = async (message) => {
    ...
    let audit = await message.guild.fetchAuditLogs({ limit: 5 });
    let entry = audit.entries.first();

    if(data && entry && entry.executor && entry.action == 'MESSAGE_DELETE' && entry.extra && entry.extra.channel && entry.target && entry.extra.channel.id == message.channel.id && entry.target.id == message.author.id && !OtherBots.includes(entry.executor.id) && entry.executor.id !== client.user.id) {
