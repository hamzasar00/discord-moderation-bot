...
const { registerSlashCommands } = require('../handlers/slashCommandHandler.js');
require('moment-duration-format');
moment.locale('tr');

module.exports = async () => {

    if (client.__vortexReadyStarted) return;
    client.__vortexReadyStarted = true;
    ...
    try {
        ...
        console.error('[SLASH] Komutlar kaydedilemedi', error);
    }
...
//Reload
    ...
    if(data) {
        ...
        const reloadChannel = client.channels.cache.get(data.channelID);
        if (reloadChannel) reloadChannel.messages.fetch(data.messageID).then(async msg => {
        ...
        else await reload.findOneAndDelete({ type: "moderation" });
    ...
    if (!guild) {
        console.error(`[BOT] Configured guild not found: ${guildID || '(empty guild ID)'}`);
        return;
    }
        ...
        if(guild && datas.length) {
            ...
            datas.filter(data => data.finishDate && data.finishDate < Date.now()).forEach(async (data, index) => {
                ...
                if(data.type == 'TEMP-JAIL') {
                    ...
                    if(jail.log && channel && member && staff) channel.send(Embed.setColor('#00FF00').setAuthor(member.user.username, member.user.avatarURL({ dynamic: true })).setDescription(`
...
**Jaillenme Sebebi :** \`${!data.reason ? 'Belirtilmedi!' : data.reason}\`
                    ...
                    if(dmMessages && member && staff) user.send(`${jailed ? jailed : ``} \`${member.guild.name}\` sunucusunda, **${staff.tag}** tarafından, ${!data.reason ? '' : `\`${data.reason}\` sebebiyle`} aldığınız **temp-jail** cezasının süresi bitti! \`(Ceza ID : #${data.id})\``).catch(() => {});
                ...
                if(data.type == 'CHAT-MUTE') {
                    ...
                    if(chatMute.log && channel && member && staff) channel.send(Embed.setColor('#00FF00').setAuthor(member.user.username, member.user.avatarURL({ dynamic: true })).setDescription(`
...
**Susturulma Sebebi :** \`${!data.reason ? 'Belirtilmedi!' : data.reason}\`
                    ...
                    if(dmMessages && member && staff) user.send(`${unCMuted ? unCMuted : `:speech_balloon:`} \`${member.guild.name}\` sunucusunda, **${staff.tag}** tarafından, ${!data.reason ? '' : `\`${data.reason}\` sebebiyle`} **metin kanallarında** aldığınız susturulma cezasının süresi bitti! \`(Ceza ID : #${data.id})\``).catch(() => {});
                ...
                if(data.type == 'VOICE-MUTE') {
                    ...
                    if(voiceMute.log && channel && member && staff) channel.send(Embed.setColor('#00FF00').setAuthor(member.user.username, member.user.avatarURL({ dynamic: true })).setDescription(`
...
**Susturulma Sebebi :** \`${!data.reason ? 'Belirtilmedi!' : data.reason}\`
                    ...
                    if(dmMessages && member && staff) user.send(`${unMuted ? unMuted : ``} \`${member.guild.name}\` sunucusunda, **${staff.tag}** tarafından, ${!data.reason ? '' : `\`${data.reason}\` sebebiyle`} **ses kanallarında** aldığınız susturulma cezasının süresi bitti! \`(Ceza ID : #${data.id})\``).catch(() => {});
