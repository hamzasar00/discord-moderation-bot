...
module.exports = async (guild, user) => {
    ...
    let audit = await guild.fetchAuditLogs({ type: 'MEMBER_BAN_ADD', limit: 5 });
    let entry = audit.entries.first();
    let executor = entry && entry.executor;
    let bannedUser = await client.fetchBan(guild, user.id);
    let reason = bannedUser && bannedUser.reason;
    ...
    if(logChannel) logChannel.send(Embed.setFooter('').setImage(banGifs.random()).setDescription(`
...
**Yasaklanma Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
    ...
    if(logs.pointLog && client.channels.cache.get(logs.pointLog)) client.channels.cache.get(logs.pointLog).send(`${user.toString()}, aldığınız \`#${penal.id}\` ID'li **Ban** cezası ile toplam **${point.penalPoint}** ceza puanına ulaştınız!`);
