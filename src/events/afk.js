...
module.exports = async (message) => {
    ...
    await Promise.all(message.mentions.members.map(async member => {
        ...
        if(memberData) {
            ...
            await message.channel.send(Embed.setDescription(`${member.toString()} kullanıcısı ${memberData.reason == 'Belirtilmedi' ? '' : `\`${memberData.reason}\` nedeniyle,`} **${moment.duration(Date.now() - memberData.date).format("d [gün] H [saat], m [dakika] s [saniye]")}** önce afk oldu!`));
