...
module.exports = async (oldUser, newUser) => {
    ...
    if(data && data.forbiddenTags.length && data.forbiddenTags.some(tag => !oldUser.username.includes(tag) && newUser.username.includes(tag))) {
        ...
        setTimeout(() => {
            ...
            const forbidChannelObject = guild.channels.cache.get(forbidChannel);
            const forbidLogObject = guild.channels.cache.get(forbidLog);
            if (forbidChannelObject) forbidChannelObject.send(`${newUser.toString()}, Maalesef yeni kullanıcı adın sunucumuzdaki yasaklı taglardan birini ( \`${tag}\` ) içeriyor. Bu sebeple sunucuya erişemezsin!`);
            if (forbidLogObject) forbidLogObject.send(`\`${member.user.tag} (${member.user.id})\` kullanıcısının sunucuya erişimi kesildi! \`(Tag: ${tag})\``);
        ...
        setTimeout(() => {
            ...
            const forbidLogObject = guild.channels.cache.get(forbidLog);
            if (forbidLogObject) forbidLogObject.send(`\`${member.user.tag} (${member.user.id})\` kullanıcısının sunucuya erişimi açıldı! \`(Tag: ${tag})\``);
