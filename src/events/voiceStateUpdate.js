...
module.exports = async (oldState, newState) => {
    ...
    if(penal) {
        ...
        if(!newState.serverMute) await newState.setMute(true);
        if(!vmuteRoles.some(role => newState.member.roles.cache.has(role))) await newState.member.roles.add(vmuteRoles);
    ...
    if(penal2) {
        ...
        if(vmuteRoles.some(role => newState.member.roles.cache.has(role))) await newState.member.roles.remove(vmuteRoles);
    ...
    if(voiceLog) {
        ...
        if(oldState.channel && !newState.channel) {
            ...
            if(entry && entry.executor && entry.action == 'MEMBER_DISCONNECT' && entry.executor.id !== client.user.id) {
                const executorMember = newState.guild.members.cache.get(entry.executor.id);
                return vLog.send(`${leaved ? leaved : ``} \`${newState.member.displayName}\` üyesinin ${oldState.channel.toString()} adlı ses kanalındaki bağlantısı \`${executorMember ? executorMember.displayName : entry.executor.tag}\` tarafından **kesildi!**`);
            }
            if(entry && entry.executor && entry.action == 'MEMBER_DISCONNECT' && entry.executor.id == client.user.id) return;
        ...
        if(oldState.channel.id && newState.channel.id && oldState.channel.id !== newState.channel.id) {
            ...
            let audit = await newState.guild.fetchAuditLogs();
            let entry = audit.entries.first();
            if(entry && entry.executor && entry.action == 'MEMBER_MOVE' && entry.executor.id !== newState.id && entry.executor.id !== client.user.id) {
                const executorMember = newState.guild.members.cache.get(entry.executor.id);
                return vLog.send(`${changeState ? changeState : `:arrow_up_down:`} \`${newState.member.displayName}\` üyesi \`${executorMember ? executorMember.displayName : entry.executor.tag}\` tarafından ${oldState.channel.toString()} adlı ses kanalından ${newState.channel.toString()} adlı ses kanalına **taşındı!**`);
            }
            
            if(!entry || !entry.executor || entry.executor.id !== client.user.id) return vLog.send(`${changeState ? changeState : `:arrow_up_down:`} \`${newState.member.displayName}\` üyesi ${oldState.channel.toString()} adlı ses kanalından ${newState.channel.toString()} adlı ses kanalına **geçiş yaptı!`);
        ...
        if(oldState.channel.id && oldState.serverMute && !newState.serverMute) {
            ...
            if(executor.id == global.client.user.id) return;
                ...
                const executorMember = newState.guild.members.cache.get(executor.id);
                return vLog.send(`${unMuted ? unMuted : ``} \`${newState.member.displayName}\` üyesinin ${newState.channel.toString()} adlı ses kanalındaki susturulması \`${executorMember ? executorMember.displayName : executor.tag}\` tarafından **kaldırıldı!**`);
            }
        ...
        if(oldState.channel.id && !oldState.serverMute && newState.serverMute) {
            ...
            if(executor.id == global.client.user.id) return;
                ...
                const executorMember = newState.guild.members.cache.get(executor.id);
                return vLog.send(`${muted ? muted : ``} \`${newState.member.displayName}\` üyesi \`${executorMember ? executorMember.displayName : executor.tag}\` tarafından ${newState.channel.toString()} adlı ses kanalında **susturuldu!**`);
            }
        ...
        if(oldState.channel.id && oldState.serverDeaf && !newState.serverDeaf) {
            ...
            if(!executor) return;
            const executorMember = newState.guild.members.cache.get(executor.id);
            return vLog.send(`${unDeafen ? unDeafen : ``} \`${newState.member.displayName}\` üyesinin ${newState.channel.toString()} adlı ses kanalındaki sağırlığı \`${executorMember ? executorMember.displayName : executor.tag}\` tarafından **kaldırıldı!**`);
        ...
        if(oldState.channel.id && !oldState.serverDeaf && newState.serverDeaf) {
            ...
            if(!executor) return;
            const executorMember = newState.guild.members.cache.get(executor.id);
            return vLog.send(`${deafen ? deafen : ``} \`${newState.member.displayName}\` üyesi \`${executorMember ? executorMember.displayName : executor.tag}\` tarafından ${newState.channel.toString()} adlı ses kanalında **sağırlaştırıldı!**`);
