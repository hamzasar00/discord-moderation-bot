...
module.exports = async (member) => {
    ...
    if(data && data.forbiddenTags.length && data.forbiddenTags.some(tag => member.user.username.includes(tag))) {
        ...
        await client.wait(2000).then(async () => {

            if(member.manageable) await member.roles.set(forbidRoles);
            let tag = data.forbiddenTags.find(tag => member.user.username.includes(tag));
            const forbidChannelObject = member.guild.channels.cache.get(forbidChannel);
            const forbidLogObject = member.guild.channels.cache.get(forbidLog);
            if (forbidChannelObject) await forbidChannelObject.send(`${member.toString()} sunucumuza hoş geldin. Maalesef kullanıcı adın sunucumuzdaki yasaklı taglardan birini ( \`${tag}\` ) içeriyor. Bu sebeple sunucuya erişemezsin!`);
            if (forbidLogObject) await forbidLogObject.send(`\`${member.user.tag} (${member.user.id})\` kullanıcısının sunucuya erişimi kesildi! \`(Tag: ${tag})\``);
