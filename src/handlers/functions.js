...
const penalCounters = require('../schemas/penalCounters.js');
...
module.exports = async (client) => {
    ...
    client.newPenal = async (guildID = client.guildSettings.guildID, userID, type, active = true, staffID, reason, temp = false, date = Date.now(), finishDate = undefined) => {
        ...
        const latestPenal = await penals.findOne({ guildID }).sort({ id: -1 }).select('id').lean();
        await penalCounters.findOneAndUpdate(
            { guildID },
            { $setOnInsert: { value: latestPenal ? latestPenal.id : 0 } },
            { upsert: true, new: true, setDefaultsOnInsert: true }
        );
        const counter = await penalCounters.findOneAndUpdate(
            { guildID },
            { $inc: { value: 1 } },
            { new: true }
        );
